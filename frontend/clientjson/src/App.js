...
function App() {
  ...
  const [novoEstudante, setNovoEstudante] = useState({ nome: '', dataNascimento: '' });
  ...
  const AdicionarEstudante = async () => {
    try {
      ...
      if (estudanteSelecionado) {
        await axios.put(`${apiURL}/estudantes/${estudanteSelecionado.id}`, novoEstudanteFormatado);
      } else {
  ...
  const EditarEstudante = (estudante) => {
    ...
    const dataNascimentoFormatada = estudante.dataNascimento
      ...
      .reverse()
      .join('-');
    setNovoEstudante({ nome: estudante.nome, dataNascimento: dataNascimentoFormatada });
  };
    ...
    <div className="container">
        ...
        <tbody>
          {estudantes.map((estudante) => (
            <tr key={estudante.id}>
              ...
              <td>
                <button className="editButton" onClick={() => EditarEstudante(estudante)}>
                  Editar
                </button>

                <button className="removeButton" onClick={() => RemoverEstudante(estudante.id)}>
                  Remover
                </button>
      ...
      <div className="form">
        ...
        <input
          ...
          onChange={(e) => setNovoEstudante({ ...novoEstudante, dataNascimento: e.target.value })}
        />
